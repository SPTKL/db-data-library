...
class Scriptor:
    ...
    To maintein the script, the current setup requires the user to go to the download page and click on the downloading link. 
    Then open up the webpage in developer mode (with Chrome in this case). Then look at the network tab and payload to find the exact
    body was generated by the click and sent as a post request to the aspx server. It is probably worth to monitor whether those 
    arguments would become stable enough that this manual process won't need to be repeated each time for update. Otherwise, alternative 
    manual download the spreadsheet and upload as local file might be preferred to current method. 
    """
